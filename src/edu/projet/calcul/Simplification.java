...
import edu.projet.interfaces.FormuleSimplification;
...
public class Simplification implements FormuleSimplification, SimplificationVisitor<Expression> {
	...
	/**
	 * Parcourt récursif de l'expression, de gauche à droite pour stocker les termes
	 * dans une liste exemple x + 1 + 2 = new Addition(new Addition(x, un), deux);
	 * x1+2+
	 *
	 */
	...
	/**
	 * Simplification des termes de la liste d'expressions générée par la
	 * méthode @see deCompose en une instance d'Expression
	 */
	private Expression simplifierTermes(List<Expression> termes) {
		...
		for (int i = 0; i < termes.size(); i++) {
			...
			// si operande -> la pile
			if (!Expression.isOperateur(termes.get(i).getSymbole())
					&& !Expression.isFonction(termes.get(i).getSymbole())) {
	...
	/**
	 * Lorsqu'un visiteur est passé à la méthode accept d'une instante de la classe
	 * Constante, la méthode visit(Constante expr) est invoquée pour cet élément
	 */
	@Override
	...
	/**
	 * Lorsqu'un visiteur est passé à la méthode accept d'une instante de la classe
	 * Variable, la méthode visit(Variable expr) est invoquée pour cet élément
	 */
	@Override
	...
	/**
	 * Lorsqu'un visiteur est passé à la méthode accept d'une instante de la classe
	 * Addition, la méthode visit(Addition expr) est invoquée pour cet élément
	 */
	@Override
	public Expression visit(Addition add) {
		...
		// e + (-1) * e = e - e = 0 -> 0
		if (add.exprD instanceof Multiplication && add.exprG.equals(add.exprD.exprD)
				&& Expression.isMoinsUn(add.exprD.exprG))
			return new Constante(0);

		// e + n*e = -> (n + 1) * e
		if (add.exprD instanceof Multiplication && Expression.isConstante(add.exprD.exprG)
				&& add.exprG.equals(add.exprD.exprD))
			return new Multiplication(new Addition(add.exprD.exprG, new Constante(1)), add.exprG).accept(this);

		// n*e + e = -> (n + 1) * e
		if (add.exprG instanceof Multiplication && Expression.isConstante(add.exprG.exprG)
				&& add.exprD.equals(add.exprG.exprD))
	...
	/**
	 * Lorsqu'un visiteur est passé à la méthode accept d'une instante de la classe
	 * Moins, la méthode visit(Moins expr) est invoquée pour cet élément
	 */
	@Override
	...
	/**
	 * Lorsqu'un visiteur est passé à la méthode accept d'une instante de la classe
	 * Multiplication, la méthode visit(Multiplication expr) est invoquée pour cet
	 * élément
	 */
	@Override
	public Expression visit(Multiplication mult) {
		...
		// (-1) * (-1) * e = e
		if (mult.exprD instanceof Multiplication && Expression.isMoinsUn(mult.exprD.exprG)
				&& Expression.isMoinsUn(mult.exprG))
		...
		// c * c' * e = (c * c') * e
		if (mult.exprD instanceof Multiplication && Expression.isConstante(mult.exprD.exprG)
				&& Expression.isConstante(mult.exprG))
			return new Multiplication(
					new Constante(((Constante) mult.exprG).getValeur() * ((Constante) mult.exprD).getValeur()),
		...
		// e^n * e^m = -> e^(n+m)
		if (mult.exprG instanceof Puissance && mult.exprD instanceof Puissance
				&& mult.exprD.exprG.equals(mult.exprG.exprG))
	...
	/**
	 * Lorsqu'un visiteur est passé à la méthode accept d'une instante de la classe
	 * Puissance, la méthode visit(Puissance expr) est invoquée pour cet élément
	 */
	@Override
	public Expression visit(Puissance pw) {
		...
		if (pw.exprD instanceof Constante && ((Constante) pw.exprD).getValeur() < 0
				&& ((Constante) pw.exprD).getValeur() != -1) {
	...
	/**
	 * Lorsqu'un visiteur est passé à la méthode accept d'une instante de la classe
	 * Log, la méthode visit(Log expr) est invoquée pour cet élément
	 */
	@Override
	...
	/**
	 * Lorsqu'un visiteur est passé à la méthode accept d'une instante de la classe
	 * Exp, la méthode visit(Exp expr) est invoquée pour cet élément
	 */
	@Override
	...
	/**
	 * Lorsqu'un visiteur est passé à la méthode accept d'une instante de la classe
	 * Cos, la méthode visit(Cos expr) est invoquée pour cet élément
	 */
	@Override
	...
	/**
	 * Lorsqu'un visiteur est passé à la méthode accept d'une instante de la classe
	 * Sin, la méthode visit(Sin expr) est invoquée pour cet élément
	 **/
	@Override
	...
	/**
	 * Lorsqu'un visiteur est passé à la méthode accept d'une instante de la classe
	 * Ch, la méthode visit(Ch expr) est invoquée pour cet élément
	 */
	@Override
	...
	/**
	 * Lorsqu'un visiteur est passé à la méthode accept d'une instante de la classe
	 * Sh, la méthode visit(Sh expr) est invoquée pour cet élément
	 **/
	@Override
