...
public class Reponse {
	...
	static List<String> fichierFiltresReponses = Fichier
			.getFichierFiltresReponses();

	/**
	 * Pile mémoire des réponses des filtres $, utilisées en cas d'absences
	 * totales de réponses
	 */
	private static Stack<String> memoireReponse;

	/**
	 * pour stocker l'index de la derniere réponse récupérée d'un filtre, 3
	 * étant la première réponse possible voir fichier filtres-reponses.csv
	 */
	 ...
	 *         regex composé, une question extraite est construite par les
	 *         résultats du match du regex sur la question de l'utilisateur. Le
	 *         premier trouvé est sélectionné. Si le match ne rend rien, on
	 *         passe au mot clé suivant.
	 */
	public static String getReponse(Map<String, Integer> motsClesQuestion,
			String question) {

		if (Professeur.activeTrace)
			System.out.println("1-class Reponse:String getReponse ("
					+ motsClesQuestion + ", " + question + ")");

		// on récupère les mots-clés de la question pour trouver la réponse
		// adéquate
		String[] reponse = {null, ""};
		String derivee;
		...
		for (Map.Entry<String, Integer> motCle : motsClesQuestion.entrySet()) {
			...
			derivee = reponse[1];
			if (reponse[0] != null) {
				// on match le tag (dv) dans la réponse pour y mettre le
				// résultat de la dérivée
				// ou rien

				if (Professeur.activeTrace)
					System.out.println(
							"1-class Reponse:String getReponse: return reponse[0].replaceAll(\"\\\\(dv\\\\)\", "
									+ derivee + ");");
	 ...
	 *         réussi, on extrait le filtre pour fabriquer l'expression
	 *         régulière (regex)
	 * 
	 ...
	 *         construit * on cherche une fonction à dériver, si oui, on garde
	 *         le résultat de la dérivation
	 * 
	 *         Si le filtre contient @ * on met dans une pile les regex
	 *         construit à partir de l'extraction des synonymes
	 *
	 *         On parcourt la pile des regex * On retire un regexde la pile * on
	 *         match * On prend au hasard une réponse dans les réponses
	 *         correspondantes au filtre choisi * Si la réponse contient goto,
	 *         on extrait le mot clé derrière le goto et on recommence * Si le
	 *         match donne une reponse non null * * si il y a une dérivée on
	 *         renvoie la réponse avec le résultat de la dérivée * * Sinon juste
	 *         la réponse * on essaie une autre regex de la pile Renvoyez la
	 *         réponse.
	 */
	...
	private static String[] getReponse(String motCleQuestion, String question) {
		...
		if (Professeur.activeTrace) {
			System.out.println("2-class Reponse:String[] getReponse ("
					+ motCleQuestion + ", " + question + ")");
			System.out.println("**********************************");
		}
		...
		for (int i = 0; i < fichierFiltresReponses.size(); i++) {
			...
			List<String> filtresReponses = Arrays
					.asList(fichierFiltresReponses.get(i).split("\\|"));

			// si le mot-clé de la question est égale au mot-clé de la ligne du
			// fichier
			if (motCleQuestion.equals(filtresReponses.get(0))) {
				...
				if (Professeur.activeTrace) {
					System.out.println("\nligne=" + i);
					System.out.println(
							"3-class Reponse:String[] getReponse motCleQuestion=filtre:("
									+ motCleQuestion + "="
									+ filtresReponses.get(0) + ")");
				}
				// on choisit la réponse suivante dans la liste des réponses du
				// filtre avant de
				...
				if (indexDernierFiltre == i) {
					...
					if (indexDerniereReponse + 1 < filtresReponses.size())
						...
						indexEnCours = indexDerniereReponse + 1
								- filtresReponses.size() + 3;
				} else {
					// sinon on choisit au hasard une réponse dans la liste des
					// réponses du filtre
					indexEnCours = (int) (Math.random()
							* (filtresReponses.size() - 3) + 3);
				...
				if (Professeur.activeTrace)
					System.out.println(
							"4-class Reponse:String[] getReponse réponse choisie:("
									+ reponse[0] + ")");
				...
				// Si la réponse contient goto, on extrait le mot clé derrière
				// le goto et on
				// recommence
				...
				//// Si le filtre contient dériver ou si on peut encore extraire
				//// une équation de
				//// la question
				if (filtre.equals("dériver") || motCleQuestion.equals("xrien")
						|| motCleQuestion.equals("fonction")) {
					// reformulation fonctions
					question = question.replaceAll("tg(\\(.*?\\))", "tan($1)");
					question = question.replaceAll("cotg(\\(.*?\\))",
							"cotan($1)");
					question = question.replaceAll("cosh(\\(.*?\\))", "ch($1)");
					question = question.replaceAll("sinh(\\(.*?\\))", "sh($1)");
					question = question.replaceAll("tanh(\\(.*?\\))", "th($1)");
					question = question.replaceAll("cotanh(\\(.*?\\))",
							"coth($1)");

					String eq = Expression.formuleToExpression(question)
							.asString();
					if (!eq.equals("0")) {
				...
				// Si le filtre contient @, on récupère tous les synonymes du
				// terme derrière @
				...
				while (regexiTerator.hasNext()) {
					...
					/*
					 * Si le match donne une reponse, on renvoie le tableau de
					 * réponse [reponse, dérivée] sinon on essaie une autre
					 * regex de la pile
					 */
					...
					if (rep != null) {
						...
						if (Professeur.activeTrace)
							System.out.println("matching(" + filtre + ","
									+ question + ")=ok");

						reponse[0] = rep;
						reponse[1] = derivee;
						return reponse;
					} else if (Professeur.activeTrace)
						System.out.println("matching(" + filtre + "," + question
								+ ")=nok");
				}
			}
		}
		...
		while (matcher.find()) {
			for (int j = 0; j <= matcher.groupCount(); j++) {
				...
				if (matcher.group(j) != null) {
					...
					String sousReponse = Conjugaison.conjuger(
							matcher.group(j).trim(),
							Fichier.getCheminFichierSujetObjet());
					// On remplace la numérotation entre parenthèses avec les
					// groupes trouvés
					reponse = reponse.replaceAll(
							"\\(" + Integer.toString(j) + "\\)",
							String.join(" ", sousReponse));
				}
			}
